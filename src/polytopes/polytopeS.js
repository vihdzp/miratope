...
function PolytopeS (symmetries, flagClasses, vertices, dimensions) {
...
PolytopeS.hypercube = function(dimensions) {
...
PolytopeS.cross = function(dimensions) {
	var symmetries = ConcreteGroup.BC(dimensions);
	var flagClasses = [];
	for(var i = 0; i < dimensions; i++) {
...
PolytopeS.recticross = function(dimensions) {
	var symmetries = ConcreteGroup.BC(dimensions);
	var flagClasses = [];
	for(var i = 0; i < dimensions; i++) {
		var row = [];
...
PolytopeS.prototype.moveFlag = function(flag, generator) {
...
PolytopeS.prototype.compareFlags = function(flag1, flag2) {
	if(flag1[0] < flag2[0])
		return -1
	if(flag1[0] > flag2[0])
		return 1
...
PolytopeS.prototype.extendSimplifier = function(simplifier, generator) {
	var newSimplifier = {};
	...
	for(var i in simplifier) {
		...
		var leftElem = this.identitySimplifier[i];
...
PolytopeS.prototype.mergeSimplifiers = function(simplifier1, simplifier2) {
	var newSimplifier = {};
...
PolytopeS.prototype.simplifierCosets = function(simplifier){
	var count = 0;
	for(var i in simplifier) {
...
PolytopeS.prototype.toPolytopeC = function(maxDomains) {
	if(!maxDomains)
