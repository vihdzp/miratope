...
export default abstract class Library {
   ...
   */
  private static checkCommonElements(a: number[], b: number[]): boolean {
    const vals: boolean[] = [];
    vals[a[0]] = true;
  ...
  static saveBlob(blob: Blob): void {
    const fileName = Library.fileName.replace("/", "_");
  ...
  static openFile(e: Event | string): void {
      ...
      switch (ext) {
        case "off":
          reader.onload = function (e: ProgressEvent<FileReader>) {
            Library.onloadOFF((e.target as FileReader).result as string);
          };
          ...
          reader.onload = function (e: ProgressEvent<FileReader>) {
            if (e.target && e.target.result)
              JSZip.loadAsync(e.target.result).then(function (zip: JSZip) {
                ...
                if (xml) xml.async("string").then(Library.onloadGGB);
    ...
    //If e is a string.
      ...
      //Extracts the filename and extension.
      const i = Library.fileName.lastIndexOf(".");
      //const ext = Library.fileName.substr(i + 1); //Extension of file.

      //Removes extension from file name.
      Library.fileName = Library.fileName.substr(0, i);

      //Lowercase name.
      if (Translation.language !== "de")
        Library.fileName = Translation.firstToLower(Library.fileName);
      ...
      xhttp.onreadystatechange = function () {
        ...
        if (this.readyState === 4 && this.status === 200)
          Library.onloadOFF(this.responseText);
      };
  ...
  static onloadGGB(contents: string): void {
    ...
    for (let i = 0; i < facets.length; i++)
      for (let j = i + 1; j < facets.length; j++)
        if (Library.checkCommonElements(facets[i], facets[j]))
  ...
  static onloadOFF(contents: string): void {
    ...
    globalThis.P = new PolytopeC(elementList, new CNName(Library.fileName));
  }
}
