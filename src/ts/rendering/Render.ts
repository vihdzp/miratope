...
export default abstract class Render {
   ...
   * [[http://geomalgorithms.com/a09-_intersect-3.html | Bentley-Ottmann
   * algorithm]] as well as a simplification algorithm to triangulate general
   * polygons.
   *
  ...
  static to(P: PolytopeB, scene: Scene): void {
    ...
    faceLoop: for (let i = 0; i < Q.elementList[2].length; i++) {
      ...
      if (Q.elementList[2][i].length < 3) continue;
  ...
  private static divide(edgeA: SweeplineEdge, edgeB: SweeplineEdge) {
    ...
    const edgeBDir = edgeB.directedEdge();
    ...
    // Add the intersection and a point at "infinitesimal distance" to the
    // vertex list. They don't actually have to be different in this
    // implementation of the algorithm. In fact, the algorithm (as implemented)
    // will fail if both nodes don't reference the same point.
    const newNode1 = new LinkedListNode(inter);
    const newNode2 = new LinkedListNode(inter);
  ...
  private static SLSort(x: SweeplineEdge, y: SweeplineEdge): number {
    ...
    const b = x.rightVertex().value;
    const c = y.leftVertex.value;
    const d = y.rightVertex().value;
    const k = Render.Event.value.coordinates[Global.index0];
