...
export default abstract class Product {
   ...
   */
  static prism(...P: PolytopeB[]): PolytopeB {
    return Product._product(P, CNType.Multiprism, Product._prism);
  }
          ...
          for (let j = 0; j < Q_.elementList[n].length; j++) {
            ...
            if (m !== 0)
              for (
                ...
                indices.push(
                  Product.getIndexOfPrismProduct(
                    m - 1,
  ...
  static tegum(...P: PolytopeB[]): PolytopeB {
    return Product._product(P, CNType.Multitegum, Product._tegum);
  }
        ...
        for (let i = 0; i < mDimCount; i++) {
          ...
          for (let j = 0; j < nDimCount; j++) {
            ...
            for (let k = 0; k < iElCount; k++) {
              ...
              indices.push(
                Product.getIndexOfTegumProduct(
                  m - 1,
                  elIndx,
                  n,
                  j,
  ...
  static pyramid(...P: PolytopeB[]): PolytopeB {
    return Product._product(P, CNType.Multipyramid, Product._pyramid);
  }
  ...
  private static _pyramid(P: PolytopeB, Q: PolytopeB): PolytopeB {
    const P_ = P.toPolytopeC();
    const Q_ = Q.toPolytopeC();
   ...
   */
  static extrudeToPrism(polytope: PolytopeB, height = 1): PolytopeB {
    return Product.prism(polytope.toPolytopeC(), Build.dyad(height));
  }
}
