...
export abstract class Render {
  ...
  static to(P_: PolytopeB, scene: Scene) {
  	...
  	function SLSort(x: SweeplineEdge, y: SweeplineEdge){
    ...
    if(!P.elementList[0] || !P.elementList[1] || !P.elementList[2]) return;
      ...
      //Every vertex should *always* have two adjacent vertices.
  		...
  		for(let j = 0; j < cycle.length - 1; j++) {
  			Render.vertexDLL[j + 1] = new LinkedListNode(P.elementList[0][cycle[j + 1]]);
  		...
  		let a = 1;
  		while(Point.equal(Render.vertexDLL[0].value, Render.vertexDLL[a].value))
  			if(++a >= Render.vertexDLL.length)
  				continue faceLoop;

  		//Tries to find three non-collinear points. If all points are collinear, doesn't render the face.
  		let b = (a === 1 ? 2 : 1);
  		while(Space.collinear(Render.vertexDLL[0].value, Render.vertexDLL[a].value, Render.vertexDLL[b].value))
        ...
        */
              ...
              return;
  				...
  				//Vertex E is a right endpoint of the edge:
  					...
  					let edge = new SweeplineEdge(Render.Event.getNode(j)!, 1 - j);
  ...
  private static _divide(edgeA: SweeplineEdge, edgeB: SweeplineEdge) {
  	...
  	let inter = Space.intersect(edgeADir[0].value, edgeADir[1].value, edgeBDir[0].value, edgeBDir[1].value);
  	if(!inter) return;
  	...
  	let newNode1 = new LinkedListNode(inter),	newNode2 = new LinkedListNode(inter);

  	Render.vertexDLL.push(newNode1); Render.vertexDLL.push(newNode2);
