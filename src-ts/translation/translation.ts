...
export abstract class Translation {
  ...
  static get(message: string, options: LanguageOptions = {}): string {
    options.count ||= 1;
    options.gender ||= "male";
  	let msg: string = Translation._globalize.messageFormatter(message)(options);
   ...
   * target language.
   */
  static addAdjective(adj: string, noun: string, options: LanguageOptions = {}) {
  	let res: string;
   ...
   * @type {Object}
   */
  private static _greekUnits: Object = {
  ...
  //Possible options:
  /* uppercase */
  static plainName(n: number, dimension: number, options: LanguageOptions = {}): string {
  	if(!options.count)
  		options.count = 1;
   ...
   */
  static elementName(n: number, options: LanguageOptions = {}) {
  	if(n > 30)
  ...
  static polytopeEnding(d: number, options: LanguageOptions = {}) {
  ...
  private static _replaceAt(str: string, index: number, replacement: string): string {
   ...
   * gender
   * count */
  static regularPolygonName(n: number, d?: number, options: LanguageOptions = {}) {
  	if(d === undefined || d === 1)
  	...
  	if(d > n / 2)
  		return Translation.addAdjective(
  			...
  			Translation.regularPolygonName(n, n - d, Translation._lowercaseLanguageOptions(options)),
   ...
   * gender
   * count */
  private static _simpleStarName(n: number, d: number, options: LanguageOptions = {}) {
  	return Translation.addAdjective(
   ...
   * gender
   * count */
  private static _starName(n: number, mod: string, options: LanguageOptions = {}) {
  ...
  private static _lowercaseLanguageOptions = function(options: LanguageOptions) {
  	let newLanguageOptions = {...options};
  ...
  private static _toAdjectiveBeforeLastWord = function(name: string, gender: string) {
  ...
  private static _toAdjectiveBeforeLastWordGendered = function(name: string, gender: string) {
  	let i = name.lastIndexOf(" ");
  ...
  private static _endings: Object = {
  	en:	[
  ...
  private static _findEnding = function (name: string, endings: Ending[]) {
  	let first: number, mid: number,	last: number,
    firstMatch: number = 0,	lastMatch: number = endings.length - 1,
    k: number = 1, //The number of characters we're checking.
    backup: number = -1;
