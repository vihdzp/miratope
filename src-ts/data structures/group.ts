...
export abstract class Group<T> {
	...
	enumerateElements(max: number = 100): T[] {
		let elems: T[] = [];
		let elemsToCheck: T[] = [this.identity()];

		whileLoop:
		while(elemsToCheck.length && elems.length < max) {
			let elem: T = elemsToCheck.pop()!;
			...
			for(let i = 0; i < this.generators.length; i++) {
				let newElem = this.multiply(elem, this.generators[i]);
...
export class MatrixGroup extends Group<Matrix> {
	...
	equal(elem1: Matrix, elem2: Matrix) {
		return (elem1.compare(elem2) === 0);
	};

	compare(elem1: Matrix, elem2: Matrix) {
...
export class ConcreteGroup<T> extends Group<[T, Matrix]> {
	...
	static BC(n: number): ConcreteGroup<string> {
		let symmetryGens: Matrix[] = [];
		//TODO: Find a better way to make these matrices
		let fourNode = Matrix.identity(n);
		fourNode.els[0][0] = -1;
		symmetryGens.push(fourNode);
		for(let i = 0; i < n - 1; i++) {
			let flipNode = Matrix.identity(n);
			flipNode.els[i][i] = 0;
		...
		let abstractSymmetries = new RewriteGroup([..."0123"], [["00",""],["11",""],["22",""],["33",""],["1010","0101"],["20","02"],["30","03"],["212","121"],["31","13"],["323","232"],["2102","1210"],["3213","2321"],["32103","23210"],["210121","121012"],["321013","232101"],["32101232","23210123"]]);
