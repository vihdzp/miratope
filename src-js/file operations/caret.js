...
class Caret {
     ...
     * console.log(C.pos);
     */
    skipToStringList(strs) {
        let matches = []; //Counts the amount of matching letters per string.
        while (!this.EOF) {
            let c = this.getChar();
            //Updates matches.
     ...
     */
    throwError(code) {
        throw new Error(translation_1.Translation.get("error/" + code, {
            line: this.line,
            column: this.column
        }));
    }
